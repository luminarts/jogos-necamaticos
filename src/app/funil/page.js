    ...
    <div className="flex justify-center items-center dark:bg-black dark:text-white">
      <div className="h-screen flex flex-col justify-center items-center">
        ...
        <div className="flex flex-col justify-center items-center">
          ...
          {level > 11 && level <= 13 && <div className="border-2 p-2 bg-white text-black dark:bg-black dark:text-white">{displayShuffles[1].map(function(number) { return number + 1})}</div>}
          <ShuffleOptions answer={displayShuffles[answerIndex]} handleUserSelection={handleUserSelection} level={level}></ShuffleOptions>
          {((level > 3 && level <= 5) || (level > 7)) && <div className="border-2 p-2 m-2 bg-white text-black dark:bg-black dark:text-white">{displayShuffles[1].map(function(number) { return number + 1})}</div>}
          {level > 7 && level <= 9 && <div className="border-2 p-2 m-2 bg-white text-black dark:bg-black dark:text-white">{displayShuffles[2].map(function(number) { return number + 1})}</div>} 
          <div className="flex flex-row m-4 gap-4 bg-gray-600 p-3 rounded">
